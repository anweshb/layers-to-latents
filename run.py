...
class Evaluator:
    def __init__(self, config):
        ...
        # # self.model = AutoModelForCausalLM.from_pretrained("/home/mrinal/scratch/layers-to-latents/outputs2/checkpoint-468").to(self.device).eval()
        # from model_moh import LlamaForCausalLM, LlamaAttention
        # # self.model = LlamaForCausalLM.from_pretrained("/home/mrinal/scratch/layers-to-latents/outputs2/checkpoint-468").to(self.device).eval()
        # self.model = LlamaForCausalLM.from_pretrained(self.model_name).to(self.device).eval()
        # for i in range(self.model.config.num_hidden_layers):
        #     attention_layer = LlamaAttention(self.model.config, i)
        #     # attention_layer.q_proj.weight.data.copy_(self.model.model.layers[i].self_attn.q_proj.weight.data)
        #     # attention_layer.k_proj.weight.data.copy_(self.model.model.layers[i].self_attn.k_proj.weight.data)
        #     # attention_layer.v_proj.weight.data.copy_(self.model.model.layers[i].self_attn.v_proj.weight.data)
        #     # attention_layer.o_proj.weight.data.copy_(self.model.model.layers[i].self_attn.o_proj.weight.data)
        #     self.model.model.layers[i].self_attn = attention_layer    

        self.model = AutoModelForCausalLM.from_pretrained("/home/mrinal/scratch/layers-to-latents/outputs/checkpoint-800", trust_remote_code=True)
    ...
    def get_prompt_and_label_gsm8k(self, row):
        ...
        # """
        prompt = """
        You are a careful, reliable math-reasoning assistant. You solve grade-school word problems using precise reasoning. Think step by step.

        **Instructions (follow exactly)**
        1. Show concise, step-by-step reasoning — each step must contain a arithmetic or logical operation.
        2. Put the final answer on its own line inside \\boxed{...} and label it exactly: FINAL ANSWER: before the box.
        3. End your response immediately after the boxed answer. No extra text, no commentary.

        ### Examples (follow formatting exactly)
        ...
        Step 1: Tuesday = 2 × 15 = 30.  
        Step 2: Wednesday = 30 − 10 = 20.  
        Step 3: Total = 15 + 30 + 20 = 65. 
        FINAL ANSWER: \\boxed{65}

        Q: A toy costs $14. Emma buys 4 of them and pays with a $100 bill. How much change does she get?

        Step 1: Cost = 4 × 14 = 56.  
        Step 2: Change = 100 − 56 = 44.
        FINAL ANSWER: \\boxed{44}

        Q: A school has 280 students. 3/4 of them take the bus. How many students take the bus?

        Step 1: 3/4 × 280 = (3 × 280) ÷ 4 = 840 ÷ 4 = 210.  
        FINAL ANSWER: \\boxed{210}

        You will now solve new problems following this exact format.
    ...
    def evaluate(self):
        ...
        for row in tqdm(self.dataset, desc="Evaluating dataset"):
            ...
            if self.dataset_name == "mmlu":
                ...
                with torch.no_grad():
                    outputs = self.model.generate(**tokenized_inputs, max_new_tokens=2048, use_cache=False, temperature=0.6, top_p=0.95, top_k=20, min_p=0.0)
