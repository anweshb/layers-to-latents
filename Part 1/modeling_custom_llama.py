...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        #################MoH#################
        self.router_mlp_moh = nn.Sequential(nn.Linear(self.head_dim, self.num_heads))
    ...
    def forward(
        ...
        **kwargs,
        ...
        router_logits = self.router_mlp_moh(query_states.view(-1, self.num_heads, self.head_dim))
        logits = torch.stack([torch.norm(router_logits[:, i, :], p=2, dim=1) for i in range(self.shared_head, self.num_heads)],
                             dim=1)
        ...
        padding_mask = kwargs.get("padding_mask", None).view(-1)
        if padding_mask is not None:
            pi = gates[~padding_mask].mean(dim=0)
            fi = mask.squeeze(0)[~padding_mask].float().mean(dim=0)
            load_balancing_loss = torch.sum(pi * fi[self.shared_head:])
            if l_aux is None:
                l_aux = load_balancing_loss
            else:
                l_aux += load_balancing_loss
